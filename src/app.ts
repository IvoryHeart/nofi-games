  ...
  private async showHome(): Promise<void> {
    ...
    this.root.innerHTML = `
      <nav class="header" role="navigation">
        <div class="header-actions header-actions-vertical">
    ...
    this.root.querySelector('#share-btn')!.addEventListener('click', () => this.shareHome());
    ...
    sorted.forEach((game, i) => {
      ...
      card.innerHTML = `
        ...
        <div class="game-card-thumb" style="background:${bg}">
          ...
          <nav class="header" role="navigation">
            ...
            <div class="header-actions header-actions-diff">
              <div class="header-actions-row">
                <button class="header-back" id="diff-fav" style="background:${isFav ? '#F5A623' : 'rgba(0,0,0,0.2)'}; font-size:28px;" aria-label="${isFav ? 'Remove from favourites' : 'Add to favourites'}">${isFav ? '\u2605' : '\u2606'}</button>
        ...
        const pollStats = (): void => {
          ...
          if (stats2.length === 0) {
            ...
            statsEl.innerHTML = stats2.map(s =>
              `<div class="hud-stat-label">${s.label}</div><div class="hud-stat-value">${s.value}</div>`
            ).join('');
          }
