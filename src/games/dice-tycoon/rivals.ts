 ...
 * Coins and shields scale with boardLevel. Names are picked without
 * repetition from the pool.
 */
export function generateRivals(rng: () => number, boardLevel: number): Rival[] {
  ...
  for (let i = 0; i < chosen.length; i++) {
    // Base pile grows with level; a seeded jitter keeps rivals distinct.
    const base = 200 * level;
    const jitter = randInt(rng, 200 * level + 1); // 0 .. 200*level
    const coins = nonNegInt(base + jitter);
 ...
 * stolen). `multiplier` (1/3/10) scales the stolen amount.
 *
...
export function resolveRaid(
  ...
  // Sanitize multiplier (the game uses 1/3/10; allow any finite >= 1).
  ...
  const stolen = Math.max(0, Math.min(rival.coins, nonNegInt(rawSteal)));
...
export function resolveCounterRaid(
  ...
  // Player loses 8%..25% of their coins, clamped to what they have.
  const frac = 0.08 + safeRng(rng) * 0.17;
  const rawLoss = Math.floor(coins * frac);
