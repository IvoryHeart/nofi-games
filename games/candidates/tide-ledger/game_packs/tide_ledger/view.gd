...
func _draw() -> void:
    ...
    draw_string(
        font,
        Vector2(66, 88),
        "Read the open routes, stamp every marker, then reach the lighthouse before the tide runs out.",
    ...
    var player_position := int(level.get("player_position", -1))
    var corridor_y := int(level.get("corridor_y", height / 2))
    ...
    for position: int in range(tiles.size()):
        ...
        var side_route := y != corridor_y and tile != 0
        var color := Color("16242b")
        if tile == 1:
            color = Color("8f7fb3") if side_route else Color("c4b88b")
        elif tile == 2:
            color = Color("73a7b5") if tide == 0 else Color("25434e")
        elif tile == 3:
            color = Color("d7bd74") if tide == 1 else Color("4b3e2c")
        draw_rect(rect, color, true)
        draw_rect(rect, Color("c5b9ed") if side_route else Color("20343a"), false, 2.0)
        if side_route and tile != 1:
            draw_circle(rect.position + rect.size * 0.5, 4.0, Color("e8dffb") if _tile_is_open(tile, tide) else Color("6c5367"))
    ...
    if goal >= 0:
        ...
        draw_rect(goal_rect, Color("f0d59a"), false, 3.0)
        var lighthouse_base := goal_rect.position + Vector2(goal_rect.size.x * 0.5, goal_rect.size.y - 7)
        draw_line(lighthouse_base, lighthouse_base - Vector2(0, goal_rect.size.y * 0.65), Color("f5f1dd"), 4.0)
        draw_colored_polygon(
            PackedVector2Array(
                [
                    lighthouse_base - Vector2(0, goal_rect.size.y * 0.65),
                    lighthouse_base + Vector2(goal_rect.size.x * 0.34, -goal_rect.size.y * 0.48),
                    lighthouse_base + Vector2(goal_rect.size.x * 0.34, -goal_rect.size.y * 0.28),
                ]
            ),
            Color("f0d59a"),
        )
    ...
    if player_position >= 0:
        ...
        draw_line(player_center + Vector2(-cell_size * 0.18, cell_size * 0.18), player_center + Vector2(-cell_size * 0.34, cell_size * 0.32), Color("f5f1dd"), 3.0)
        draw_line(player_center + Vector2(cell_size * 0.18, cell_size * 0.18), player_center + Vector2(cell_size * 0.34, cell_size * 0.32), Color("f5f1dd"), 3.0)
    ...
    var actions_applied := int(_state.get("actions_applied", 0))
    var action_limit := int(_state.get("action_limit", 0))
    var remaining_actions := maxi(0, action_limit - actions_applied)
    draw_string(font, Vector2(64, 624), "Arrows / WASD: move     Space: flip tide     R: restart     Tap a route tile or visible control", HORIZONTAL_ALIGNMENT_LEFT, -1, 17, Color("9eb4b2"))
    draw_string(font, Vector2(760, 624), "Actions remaining: %d / %d" % [remaining_actions, action_limit], HORIZONTAL_ALIGNMENT_LEFT, 450, 18, Color("e8f0e7"))
    draw_rect(Rect2(760, 635, 450, 6), Color("1d2c31"), true)
    var budget_ratio := float(remaining_actions) / float(maxi(1, action_limit))
    draw_rect(Rect2(760, 635, 450.0 * budget_ratio, 6), Color("78b9a8") if budget_ratio > 0.35 else Color("d46b5f"), true)
    ...
    draw_string(font, Vector2(64, 690), "Blue / gold: open in current tide    Purple edge: side route    Red marker: still needed", HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color("9eb4b2"))
...
func _tile_is_open(tile: int, tide: int) -> bool:
    return tile == 1 or (tile == 2 and tide == 0) or (tile == 3 and tide == 1)
