...
func _draw() -> void:
    ...
    draw_string(font, Vector2(64, 624), "Arrows / WASD: move     Space: flip tide     R: restart     Touch a neighboring tile", HORIZONTAL_ALIGNMENT_LEFT, -1, 17, Color("9eb4b2"))
    var status := "Markers: %d / %d" % [int(_state.get("marker_count", 0)), markers.size()]
    if bool(_state.get("complete", false)):
        status = "Ledger stamped. Press N or tap the shore to chart the next level."
