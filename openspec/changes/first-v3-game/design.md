...
The three bands are `shoal`, `swell`, and `storm`. Their bounds are versioned constants in Tide Ledger rather than a platform-wide policy. The solution-length ranges allow the required side-branch detour:
...
The candidate's headless runner will exercise the SDK validator, same-seed reset, generated LevelSpec hashes, immutable accessor behavior, solver proofs, required off-corridor detours, difficulty bounds, fail-closed fallback behavior, applicable and invalid actions, objective completion/failure, endless-level reconstruction, every-corpus replay restoration, and replay round trips. Repository checks will additionally validate the catalog, SDK synchronization, fixture behavior, player pack loading, and web export. This gives falsifiable acceptance at the pack seam without creating an evaluator framework.
...
The change is accepted only if Tide Ledger's headless test passes the semantic contract, deterministic reset, immutable LevelSpec accessors, generator corpus, solver proofs, required off-corridor detours, three-band difficulty bounds, 32-attempt/fail-closed fallback behavior across the representative fallback seed range, applicable actions, objectives, metrics, endless-level reconstruction, every-corpus replay round trips, atomic rejected-replay checks, and pack-boundary generation-failure checks; the development catalog validates with exactly one discoverable product pack plus a hidden fixture; the player loads the hash-pinned pack through its existing loader; `pnpm check` and `pnpm build:web` pass; the exported preview can be inspected with the documented keyboard and visible touch controls; and a human passes the final playability gate before archival. Any missing or ambiguous result is a human-review blocker, not permission to lower the bar.
