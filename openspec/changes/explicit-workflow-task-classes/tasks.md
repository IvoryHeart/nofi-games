...
- Task identity: `workflow-task-classes/plan`; implementation must preserve this change and update the task identity explicitly if reassigned.
...
- [ ] 1.1 Search all supported workflow-definition consumers and any persisted/external schema-version-1 assumptions; record the result, and stop for a high-judgment design update if an undeclared consumer makes the atomic version-2 migration unsafe.
- [ ] 1.2 Add the exported two-value `TaskClass` Zod enum and inferred TypeScript type, require `taskClass` on `WorkflowStage`, and change only the workflow-definition schema boundary to `schemaVersion: 2`.
- [ ] 1.3 Update `research-to-game.yaml`, `game-improvement.yaml`, and `agent-evolution.yaml` atomically to schema version 2 and workflow version `0.2.0`, adding the exact 13-stage classification from the delta spec without model or reasoning identifiers.
- [ ] 1.4 Add the `mechanically-verifiable-edit` gate to `challenger-build` and verify no other dependency, artifact, agent, publication, or separation-of-duties behavior changed.

## 2. Deterministic tests and operator documentation

- [ ] 2.1 Add focused contract tests that accept both task classes and reject a missing value, `balanced`, `bounded`, a model identifier, and schema version 1 under the new contract.
- [ ] 2.2 Add a table-driven test that loads all three manifests and asserts the exact workflow/stage/task-class matrix, schema/workflow versions, total stage count, and challenger exact-edit gate.
- [ ] 2.3 Preserve the existing repository validator's agent, skill, dependency, and artifact checks; run its positive path against the migrated manifests and do not create a second task-class vocabulary.
- [ ] 2.4 Update the native-harness runbook so a bounded task packet cites accepted/frozen inputs, narrow writable scope, mechanical checks, rollback, and manifest-declared class; document checkpoint/high-judgment escalation and the challenger exact-edit condition without duplicating model defaults into workflows.

## 3. Verification and strategic-fit evaluation

- [ ] 3.1 Run the focused task-class test and `pnpm workflows:validate`; record full results, the 13-row mapping, and every negative fixture outcome in `verification.md`.
- [ ] 3.2 Run strict change validation and `pnpm check`; resolve every failure within scope or record a blocker, with no ignored or relabeled errors.
- [ ] 3.3 Compare the implementation diff with `98ce6ce5219685d4dd6d8715cae6e3c521faed74`; record changed paths and hashes, prove manifests contain no model/reasoning/provider/session/router/scheduler/lease fields, and prove no forbidden subsystem, dependency, database, player, game-pack, catalog, or distribution change was added.
- [ ] 3.4 Exercise every adversarial case in `evaluation-plan.md`, including ambiguous challenger edits and bounded tasks that discover design, policy, risk, scope, architecture, or promotion decisions; record checkpoint-and-escalation outcomes rather than self-reported compliance.
- [ ] 3.5 Reproduce or inspect the Git-only rollback in an isolated snapshot if required by the independent reviewer, confirming that the pinned target restores contract version 1 and all three version-0.1.0 manifests without external cleanup.

## 4. Evidence closure and independent decision

- [ ] 4.1 Complete `verification.md` with execution provenance, raw evidence references, requirement-by-requirement results, protected metrics, failures, unresolved work, rollback evidence, and the planning retry hash above.
- [ ] 4.2 Complete `retrospective.md` with evidence-backed reusable lessons and invalidated assumptions, or explicitly state that none were found; do not convert unsupported suggestions into policy.
...
- [ ] 4.4 Commit a coherent implementation/evidence checkpoint only after all applicable protected metrics pass; leave the change unarchived until verification, retrospective, and independent decision satisfy the system-change schema.
